...
class Database:
    def __init__(self):
        self.db = psycopg2.connect(database="courses_bot", user="rafailvv",
                       password="QwErT1234", host="195.2.81.248", port="5432")
        # self.db = sqlite3.connect('bot/database/education.db')
        self.cur = self.db.cursor()

    def get_courses_name(self):
        courses_names = []
        self.cur.execute("SELECT name FROM course")
        for name in self.cur.fetchall():
            courses_names.append(name[0])
        return courses_names

    def get_course_program(self, course_id):
        self.cur.execute(f"SELECT program FROM course WHERE id = '{course_id}'")
        return open(f'bot/image/course_programs/{self.cur.fetchone()[0]}', 'rb')

    def get_course_name_by_course_id(self, course_id):
        self.cur.execute(f"SELECT name FROM course WHERE id = '{course_id}'")
        return self.cur.fetchone()[0]

    def get_course_full_description(self, course_id):
        self.cur.execute(f"SELECT short_description,description FROM course WHERE id = '{course_id}'")
        des_query = self.cur.fetchone()
        return des_query[0] + "\n\n" + des_query[1]

    def get_course_short_description(self, course_id):
        self.cur.execute(f"SELECT short_description FROM course WHERE id = '{course_id}'")
        return self.cur.fetchone()[0]
    ...
    def get_near_course_by_name(self, course_name):
        ...
        for course, flow_date in flows:
            delta = (flow_date - datetime.now().date()).days
    ...
    def is_teacher(self, chat_id, useraname):
        self.cur.execute(f"SELECT id FROM teacher WHERE username = '@{useraname}'")
    ...
    def is_student(self, chat_id, useraname):
        self.cur.execute(f"SELECT id FROM student WHERE username = '@{useraname}' AND is_approved = True")
        id = self.cur.fetchone()
        if id is not None:
    ...
    def get_teacher_id_by_chat_id(self, chat_id):
        self.cur.execute(f"SELECT id FROM teacher WHERE chat_id = {chat_id}")
        return self.cur.fetchone()[0]

    def get_student_id_by_chat_id(self, chat_id):
        self.cur.execute(f"SELECT id FROM student WHERE chat_id = {chat_id}")
        return self.cur.fetchone()[0]
    ...
    def get_further_flows_for_teacher(self, teacher_id):
        ...
        self.cur.execute(f"""
                    ...
                    FROM course_flow 
                    INNER JOIN course c on course_flow.course = c.id AND course_flow.teacher = {teacher_id} """)
        flows = self.cur.fetchall()
        for flow in flows:
            res.append((flow[0], datetime.strftime(flow[1], "%d.%m.%Y"), datetime.strftime(flow[2], "%d.%m.%Y")))
        return res
    ...
    def get_current_flows_for_student(self, student_id):
        ...
        self.cur.execute(f"""
                    ...
                    INNER JOIN student s on course_flow.id = s.course_flow AND s.id = {student_id}""")
        flows = self.cur.fetchall()
        for flow in flows:
            res.append((flow[0], datetime.strftime(flow[1], "%d.%m.%Y"), datetime.strftime(flow[2], "%d.%m.%Y")))
        return res
    ...
    def get_near_course_flow_by_course_id(self, id):
        min_delta = -1
        self.cur.execute(f"SELECT id, start_date FROM course_flow WHERE course = {id} AND student_count < 7")
        for course_flow, flow_date in self.cur.fetchall():
            delta = (flow_date - datetime.now().date()).days
        ...
        self.cur.execute(f"""
                    ...
                    WHERE course = (SELECT id FROM course WHERE name = '{course_name}') AND
                        ...
                        finish_date = '{datetime(int(finish_date.split(".")[2]),
                                                 ...
                                                 int(finish_date.split(".")[0])).date()}'""")
        return self.cur.fetchone()[0]

    def get_link_by_flow_id(self, id):
        self.cur.execute(f"SELECT lesson_link FROM course_flow WHERE id = {id}")
        return self.cur.fetchone()[0]

    def get_list_students_by_flow_id(self, id):
        self.cur.execute(f"SELECT full_name, id FROM student WHERE course_flow = {id} AND is_approved = True")
        return self.cur.fetchall()

    def get_chat_id_students_in_flow(self, id):
        self.cur.execute(f"SELECT chat_id FROM student WHERE course_flow = {id} AND is_approved = True")
        return self.cur.fetchall()

    def get_student_info(self, student_id):
        self.cur.execute(f"SELECT full_name, username, phone FROM student WHERE id = {student_id} AND is_approved = True")
    ...
    def get_teacher_name_by_chat_id(self, chat_id):
        self.cur.execute(f"SELECT full_name FROM teacher WHERE chat_id = {chat_id}")
        teacher_name = self.cur.fetchone()[
            0].split()
        if teacher_name is not None:
            return teacher_name[1]

    def get_student_name_by_chat_id(self, chat_id):
        self.cur.execute(f"SELECT full_name FROM student WHERE chat_id = {chat_id}")
        student_name = self.cur.fetchone()[0].split()
    ...
    def update_confirmation(self, student_id, is_approved=False):
        if not is_approved:
            ...
            self.cur.execute(f"UPDATE student SET is_approved = True WHERE id = {student_id}")
    ...
    def get_student_id_by_username(self, username):
        self.cur.execute(f"SELECT id FROM student WHERE username = '@{username}'")
        return self.cur.fetchone()[0]

    def get_student_chat_id_by_id(self, id):
        self.cur.execute(f"SELECT chat_id FROM student WHERE id = {id}")
    ...
    def get_timetable_by_flow_id(self, id):
        self.cur.execute(f"SELECT timetable FROM course_flow WHERE id = {id}")
        return self.cur.fetchone()[0]
    ...
    def get_teacher_info(self, flow_id):
        self.cur.execute(f"""SELECT full_name, username, phone FROM course_flow, teacher 
                                WHERE course_flow.id = {flow_id} AND teacher.id = course_flow.teacher""")
        return self.cur.fetchone()

    def save_new_recording(self, video, description, flow_id):
        self.cur.execute(f"SELECT MIN(lesson_number) FROM video_recording WHERE flow_id = {flow_id}")
        lesson_number = self.cur.fetchone()[0]
    ...
    def get_students_chat_id_in_flow(self, flow_id):
        self.cur.execute(f"SELECT chat_id FROM student WHERE course_flow = {flow_id}")
        return self.cur.fetchall()

    def get_id_recordings_by_flow_id(self, flow_id):
        self.cur.execute(f"SELECT lesson_number, id  FROM video_recording WHERE flow_id = {flow_id} ORDER BY lesson_number")
        return self.cur.fetchall()
    ...
    def get_past_lessons_numbers(self, flow_id):
        self.cur.execute(f"""SELECT lesson_number FROM video_recording WHERE flow_id = {flow_id}""")
        return self.cur.fetchall()

    def get_fio_teacher_by_chat_id(self, chat_id):
        self.cur.execute(f"""SELECT full_name FROM teacher WHERE chat_id = {chat_id}""")
        return " ".join(self.cur.fetchone()[0].split()[1:])
    ...
    def get_hw(self, id):
        self.cur.execute(f"SELECT content, content_type FROM homework WHERE id = {id}")
        return self.cur.fetchone()

    def get_hw_solution(self, id):
        self.cur.execute(f"SELECT content_solution, content_type_solution FROM homework WHERE id = {id}")
        return self.cur.fetchone()
    ...
    def get_lesson_number_by_hw_id(self, hw_id):
        self.cur.execute(f"SELECT lesson_number FROM homework WHERE id = {hw_id}")
        return self.cur.fetchone()[0]
    ...
    def get_not_done_hw_id(slef, flow_id, student_chat_id):
        slef.cur.execute(f"""
                    ...
                    AND content_solution is null""")
        return list(set (slef.cur.fetchall()))
    ...
    def get_chat_id_all_users(self):
        self.cur.execute("SELECT chat_id FROM all_users")
        return self.cur.fetchall()