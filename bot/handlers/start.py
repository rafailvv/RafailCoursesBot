...
class Start:
    ...
    async def send_updated_bot_message(self):
            ...
            for user_chat_id in chat_ids :
                ...
                try:
                     await self.bot.send_message(
                        ...
                        text=get_text_by_key('NEW_UPDATE_TEXT')
                    )
                except Exception:
    ...
    async def start_message(self, message : Message,state : FSMContext):
        ...
        if self.db.is_teacher(message.chat.id,message.from_user.username):
            ...
            if flows['keyboard']:
                await message.answer(text=get_text_by_key('TEACHER_START_TEXT').format(self.db.get_teacher_name_by_chat_id(message.chat.id)),
                                     reply_markup=flows)
            else:
            ...
            await self.bot.send_photo(chat_id=message.chat.id,
                             ...
                             caption=get_text_by_key('START_TEXT').format(message.from_user.first_name),
